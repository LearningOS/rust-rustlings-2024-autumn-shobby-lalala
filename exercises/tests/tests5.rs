...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: We assume the address is valid and points to a `u32`.
    let ptr = address as *mut u32; // Convert the address back to a mutable pointer.
    *ptr = 0xAABBCCDD; // Modify the value at that address.
}
